...
export function getGoldenHourTimes(lat, lon) {
  ...
  const morningSunPosition = SunCalc.getPosition(new Date(morningGoldenHourStart.getTime() + 30 * 60 * 1000), lat, lon)

  const eveningSunPosition = SunCalc.getPosition(new Date(eveningGoldenHourStart.getTime() + 30 * 60 * 1000), lat, lon)
